...
const make = Effect.gen(function* () {
  ...
  const materializeSpeechToTextSecrets = (
    ...
    Effect.gen(function* () {
      ...
      const secret = yield* secretStore.get(SPEECH_TO_TEXT_GROQ_API_KEY_SECRET).pipe(
        Effect.mapError(
          (cause) =>
            new ServerSettingsError({
              settingsPath,
      ...
      return {
        ...
        speechToText: {
          ...settings.speechToText,
          groqApiKey: Option.isSome(secret) ? textDecoder.decode(secret.value) : "",
        },
      };
    });
  ...
  const persistSpeechToTextSecrets = (
    ...
    Effect.gen(function* () {
      ...
      yield* secretStore.remove(SPEECH_TO_TEXT_GROQ_API_KEY_SECRET).pipe(
        Effect.mapError(
          (cause) =>
            new ServerSettingsError({
              settingsPath,
