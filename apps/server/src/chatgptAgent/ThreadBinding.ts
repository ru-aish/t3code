...
export type ChatGPTAgentThreadBindingError =
  | PersistenceSqlError
  | PersistenceDecodeError;
...
export class ChatGPTAgentThreadBindings extends Context.Service<
  ChatGPTAgentThreadBindings,
  {
    readonly get: (
      ...
      Option.Option<ChatGPTAgentThreadBinding>,
    ...
    return ChatGPTAgentThreadBindings.of({
      get: (threadId) =>
        getRow({ threadId }).pipe(
          ...
          Effect.mapError((cause) =>
            error("ChatGPTAgentThreadBindings.get", cause, threadId),
          ),
        ),
      upsert: (binding) =>
        putRow(binding).pipe(
      ...
      markWorkspaceEnvelopeSent: (threadId, sentAt) =>
        mark({ threadId, sentAt }).pipe(
          Effect.mapError((cause) =>
            error(
              "ChatGPTAgentThreadBindings.markWorkspaceEnvelopeSent",
              cause,
              threadId,
            ),
          ),
      ...
      delete: (threadId) =>
        remove({ threadId }).pipe(
          Effect.mapError((cause) =>
            error("ChatGPTAgentThreadBindings.delete", cause, threadId),
          ),
        ),
    });
  }),
